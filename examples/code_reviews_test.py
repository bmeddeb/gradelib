...
async def main():
    ...
    try:
        ...
        for repo_url, repo_result in code_reviews.items():
            ...
            review_count = sum(len(reviews)
                               for reviews in repo_result.values())
            ...
            for pr_number, reviews in repo_result.items():
                for review in reviews:
                    ...
                    review_data = {
                        ...
                        # Shortened commit ID
                        'Commit ID': review['commit_id'][:8],
                        'Body': review['body'][:50] + '...' if review['body'] and len(review['body']) > 50 else review['body'],
                        'URL': review['html_url'],
                        'Review Day': review_day,
                        # Start of the week
                        'Review Week': review_day - timedelta(days=review_day.weekday()),
                        # Start of the month
                        'Review Month': review_day.replace(day=1),
                    }
        ...
        if all_reviews:
            ...
            if 'State' in df.columns:
                ...
                if 'CHANGES_REQUESTED' in state_counts:
                    print(
                        f"Changes Requested Rate: {state_counts.get('CHANGES_REQUESTED', 0) / len(df):.2%}")
            ...
            print("\nPRs with Most Reviews:")
            pr_review_counts = df.groupby(
                ['Repository', 'PR Number']).size().sort_values(ascending=False)
            ...
            day_of_week_counts = df['Submitted At'].dt.day_name(
            ).value_counts()
            print(day_of_week_counts)

            print("\nReview Activity by Hour of Day:")
            hour_counts = df['Submitted At'].dt.hour.value_counts(
            ).sort_index()
            print(hour_counts)

            # Save to CSV
            csv_path = os.path.join(os.path.dirname(
                __file__), 'code_review_analysis.csv')
            ...
            try:
                ...
                if len(timeline_series) > 1:
                    date_range = pd.date_range(
                        start=timeline_series.index.min(), end=timeline_series.index.max())
                    timeline_series = timeline_series.reindex(
                ...
                if review_states_by_repo:
                    ...
                    plt.savefig(os.path.join(
                        output_dir, 'review_states_by_repo.png'))
                ...
                if reviewer_activity:
                    ...
                    top_reviewers = dict(
                        sorted(reviewer_activity.items(), key=lambda x: x[1], reverse=True)[:10])
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(
                    output_dir, 'day_of_week_activity.png'))
