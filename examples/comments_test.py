...
async def main():
    ...
    try:
        ...
        comments = await manager.fetch_comments(repo_urls, comment_types=None, max_pages=None)
        ...
        if all_comments:
            ...
            for comment_type, count in comment_types_count.items():
                print(
                    f"  {comment_type}: {count} ({count/total_comment_count:.1%})")
            ...
            try:
                ...
                timeline_series = pd.Series(
                    dict(comment_timeline)).sort_index()
                ...
                top_users = dict(sorted(comment_by_user.items(),
                                 key=lambda x: x[1], reverse=True)[:10])
                plt.barh(list(reversed(list(top_users.keys()))),
                         list(reversed(list(top_users.values()))))
                ...
                plt.figure(figsize=(10, 6))
                df['Repository'].value_counts().plot(
