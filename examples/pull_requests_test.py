...
async def main():
    ...
    print(
        f"Fetching pull request information for {len(repo_urls)} repositories...")
    try:
        # You can specify state=None (default), "open", "closed", or "all"
        pull_requests = await provider.fetch_pull_requests(repo_urls, state="all")
        ...
        if all_prs:
            ...
            csv_path = os.path.join(os.path.dirname(
                __file__), 'pull_request_analysis.csv')
