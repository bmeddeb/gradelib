...
2. Creates a GitHubProvider with GitHub credentials
...
async def main():
    ...
    provider = gradelib.GitHubProvider(
        github_username, github_token, repo_urls)
    ...
    try:
        ...
        issues = await provider.fetch_issues(repo_urls, state="all")
        ...
        for repo_url, repo_result in issues.items():
            ...
            actual_issues = [
                issue for issue in repo_result if not issue['is_pull_request']]
            pull_requests = [
                issue for issue in repo_result if issue['is_pull_request']]

            print(
                f"Found {len(actual_issues)} issues and {len(pull_requests)} pull requests")
