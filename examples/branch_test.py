...
async def monitor_cloning(provider, repo_urls):
        ...
        for url in repo_urls:
            if url in tasks:
                ...
                if status_type == "completed":
                    ...
                    print(
                        f"\n❌ {url} failed: {task.get('error', 'Unknown error')}")
                    completed.add(url)
                else:
                    ...
                    if status_type == "cloning" and task.get("progress") is not None:
                        ...
                        print(
                            f"\r⏳ {url} cloning: [{bar}] {percent}%", end='', flush=True)
...
async def main():
    ...
    await monitor_cloning(provider, repo_urls)

    # Analyze branches
    print("\nAnalyzing branches...")
    branches = await provider.analyze_branches(repo_urls)
